...
def fetch_weather_data(ind,countries, zones,cities):    
    try:
        ...
        for i in range(1440):
            for k in range(ind-10,len(cities)):
            ...
            print(f'Going to sleep..., time elapsed is {round(i*33/60,2)} minutes')
            sleep(1)
        ...
        cities = list(all_cities_df['city'])
        zones = list(all_cities_df['zone'])
        fetch_weather_data(index,countries,zones,cities)
    except IndexError:
...
all_cities_df = all_cities_df[all_cities_df['country'].isin(['Mexico' , 'United States'])]
